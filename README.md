...
4. **🇰🇷 API 키 없는 100% 무료 원클릭 한글 번역**:
   - 외부 번역 API 연동이나 비용 없이 각 카드별 `[한글 번역]` 및 상단 `[전체 한글 번역]` 지원.
   - 릴리즈 제목과 핵심 3줄 요약(Key Takeaways)을 실시간으로 한국어로 전환.
5. **.NET 10 Razor & GitHub Pages 라이브 대시보드**:
